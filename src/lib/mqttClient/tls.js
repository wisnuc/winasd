  ...
  */
  read (size) {
    ...
    if (this.socket.readableLength) {
      this.onData(this.socket.read(size))
    }
  }
}
...
class CertificateVerify extends HandshakeState {
  enter () {
    ...
    if (typeof key === 'function') {  // for ecc
      key(this.hs.tbs(), (err, sig) => {
        if (err) this.setState(FinalState, err)
        else {
          this.write(HandshakeType.CERTIFICATE_VERIFY,
            concat([Buffer.from([0x04, 0x03]), Prepend16(sig)]))
          this.hs.deriveKeys()
